...
This board runs `collision_classifier` (a ResNet18 free/blocked classifier via ExecuTorch,
on-device NPU inference), `oak_camera`, and `collision_avoider` on a Ventuno Q replacing a
TurtleBot 4's Raspberry Pi.
...
- **Persistent env**: `~/.ventuno_collision_avoidance_env`, sourced from `~/.bashrc` (must be sourced before
  the non-interactive-shell early-return line, since most commands run over `ssh ubuntu@ventuno 'bash -lc ...'`).
  Sets `VENTUNO_COLLISION_AVOIDANCE_ROOT`, `EXECUTORCH_ROOT`, `QNN_SDK_ROOT`, `QAIRT_LIB`, `LD_LIBRARY_PATH`,
...
| Symptom | Reference |
|---|---|
| ExecuTorch build/export fails, QNN backend errors, garbage classification scores, FastRPC/DSP errors, `find_package(executorch)` CMake errors | [references/executorch-qnn.md](references/executorch-qnn.md) |
...
2. Check the persistent env is actually loaded in the shell you're debugging in — a lot of "it worked
   over SSH but not interactively" (or vice versa) bugs are `~/.ventuno_collision_avoidance_env` not being
...
ros2 launch collision_classifier dataset_classifier.launch.py backend:=npu
```

In another shell: `ros2 topic hz /collision/classification` should show ~5 Hz with
`vision_msgs/msg/Classification` messages carrying free/blocked scores. `ros2 node list`
should show `/dataset_image_publisher` and `/collision_classifier`.
