#!/usr/bin/env bash
# Install system and project dependencies for ventuno-collision-avoidance on Ubuntu
...
PROJECT_VENV="${PROJECT_VENV:-$HOME/.venv/ventuno-collision-avoidance}"
...
write_board_environment() {
  ...
  local env_file="$TARGET_HOME/.ventuno_collision_avoidance_env"
  local bashrc="$TARGET_HOME/.bashrc"
  local tmp env_line
  tmp="$(mktemp)"

  cat > "$tmp" <<EOF
# Ventuno collision avoidance environment. Generated by scripts/install_ventuno_deps.sh.
...
export VENTUNO_COLLISION_AVOIDANCE_ROOT="$PROJECT_ROOT"
...
EOF
  ...
  env_line='[ -f "$HOME/.ventuno_collision_avoidance_env" ] && . "$HOME/.ventuno_collision_avoidance_env"'
...
Open a new shell, or run:
  ...
  source "$TARGET_HOME/.ventuno_collision_avoidance_env"
